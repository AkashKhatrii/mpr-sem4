...
router.get("/services/:type", (req, res) => {
    ...
    if(req.isAuthenticated()){
        if(req.session.searchCity){
            ...
            User.find({ service: type, city: req.session.searchCity}).then((result) => {
                res.render('services', { profession: type, services: result, city: req.session.searchCity});
            }).catch((err) => {
                console.log(err)
            })
    }else{
        ...
        User.find({ service: type }).then((result) => {
            res.render('services', { profession: type, services: result, city: "1"});
        }).catch((err) => {
            console.log(err)
        })
    }
...
router.get("/appointment/:providerId", (req, res) => {
   ...
   User.findOne({_id: req.params.providerId}).then((result) => {
        res.render("appointment", { providerId: req.params.providerId, providerName: result.custname})
    })
