...
homeRouter.post("/provider/service", (req, res) => {
    ...
    User.findOneAndUpdate({ _id: userId}, { $set: {profession: req.body.profession,
        city: req.body.city,
        experience: req.body.experience }}).then((result) => {
    ...
    if(req.isAuthenticated()){
        ...
        Appointment.find({ providerId: req.user.id}).then(( appointments ) => {
            ...
            User.findOne({ _id: req.user.id}).then((user) => {
                ...
                res.render("providerHome", { appointments: appointments, user: user })
            })
